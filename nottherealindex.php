<?php
//die("Currently Unavailable. Please check back soon.");
include("funcs.php");
...
Connect();
$_SESSION['ol'] = mysql_num_rows(Query("select * from users where logged_in = 1"));
$_SESSION['tot'] = mysql_num_rows(Query("select * from users"));
...
if($_GET['del']==1){
		...
		$res = mysql_query("delete from sighting where ct_id = ".$ctId." and user_id = '".$userID."'");
...
<div class="container">
	<div class="row"><!-- THREE COLUMNS left column for data entry, right for data display -->
		...
		<div class="col-md-9 col-lg-9 displayPanel"><!-- DATA DISPLAY -->
		      	<a class="button" href = "index.php"><i class="fa fa-refresh" aria-hidden="true"></i></a><em> for best results, use this refresh button instead of the browsers</em>
		      	<?php
			Connect();
			?>
            ...
            <th class = "arrow">loc<br /><a href = "index.php?OB=LCA"><span class="glyphicon glyphicon-arrow-up" aria-hidden="true"></span></a>
            <a href = "index.php?OB=LCD"><span class="glyphicon glyphicon-arrow-down" aria-hidden="true"></span></a></th>
            <th class = "arrow">train#<br /><a href = "index.php?OB=TNA"><span class="glyphicon glyphicon-arrow-up" aria-hidden="true"></span></a>
      ...
      if($_GET['getTrainNumber']==1){
        ...
        print"<select class = \"form-control\" onchange=\"location = this.value;\">";
          foreach($mpArray as $mp){
              print"<option value = \"index.php?getLocoNumber=1&locoNumSel=".$mp."\">$mp</option>";
          }
       print"</td>
				<td><a href = \"index.php?del=1&id=$arr[0]\">DEL</a></td>
				<td><a href = \"index.php?view=1&id=$arr[0]\">VIEW</a></td>
				</tr>";
			}
      /*
      trainNumSel
...
if($_GET['view']==1) {
	print"
	<div class=\"col-md-9 col-lg-9 viewPanel\"><!-- DATA DISPLAY -->
			<table class = \"table\">
				<thead>
					<tr>
					<th>date</th>
